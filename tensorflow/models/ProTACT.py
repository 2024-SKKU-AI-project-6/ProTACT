...
def build_ProTACT(pos_vocab_size, vocab_size, maxnum, maxlen, readability_feature_count,
    ...
    es_pr_SF_list = [SkipFlow(lstm_dim=lstm_units, k=4, maxlen=maxlen, eta=13, delta=50)(pos_avg_MA_lstm_list[i]) for i in range(output_dim)]
    es_pr_SF_lstm_list = [layers.LSTM(lstm_units, return_sequences=True)(pos_hz_SF) for pos_hz_SF in es_pr_SF_list]
