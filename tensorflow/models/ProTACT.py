...
def correlation_coefficient(trait1, trait2):
    ...
    # ignore the masked values when obtaining the mean
    mx = K.sum(x_masked) / K.sum(mask)
    # ignore the masked values when obtaining the mean
    my = K.sum(y_masked) / K.sum(mask)

    xm, ym = (x_masked-mx) * mask, (y_masked-my) * \
        mask  # maksing the masked values
...
def build_ProTACT(pos_vocab_size, vocab_size, maxnum, maxlen, readability_feature_count,
    ...
    pos_input = layers.Input(shape=(maxnum*maxlen,),
                             dtype='int32', name='pos_input')
    ...
    pos_drop_x = layers.Dropout(
        dropout_prob, name='pos_drop_x')(pos_x_maskedout)
    pos_resh_W = layers.Reshape(
        (maxnum, maxlen, embedding_dim), name='pos_resh_W')(pos_drop_x)

    pos_zcnn = layers.TimeDistributed(layers.Conv1D(
        cnn_filters, cnn_kernel_size, padding='valid'), name='pos_zcnn')(pos_resh_W)

    pos_avg_zcnn = layers.TimeDistributed(
        Attention(), name='pos_avg_zcnn')(pos_zcnn)

    linguistic_input = layers.Input(
        (linguistic_feature_count,), name='linguistic_input')
    readability_input = layers.Input(
        (readability_feature_count,), name='readability_input')

    pos_MA_list = [MultiHeadAttention(100, num_heads)(
        pos_avg_zcnn) for _ in range(output_dim)]
    pos_MA_lstm_list = [layers.LSTM(lstm_units, return_sequences=True)(
        pos_MA) for pos_MA in pos_MA_list]
    pos_avg_MA_lstm_list = [Attention()(pos_hz_lstm)
                            for pos_hz_lstm in pos_MA_lstm_list]

    # 2. Prompt Representation
    # word embedding
    prompt_word_input = layers.Input(
        shape=(maxnum*maxlen,), dtype='int32', name='prompt_word_input')
    ...
    # pos embedding
    prompt_pos_input = layers.Input(
        shape=(maxnum*maxlen,), dtype='int32', name='prompt_pos_input')
    ...
    prompt_pos_maskedout = ZeroMaskedEntries(
        name='prompt_pos_maskedout')(prompt_pos)

    # add word + pos embedding
    prompt_emb = tf.keras.layers.Add()(
        [prompt_maskedout, prompt_pos_maskedout])

    prompt_drop_x = layers.Dropout(
        dropout_prob, name='prompt_drop_x')(prompt_emb)
    prompt_resh_W = layers.Reshape(
        (maxnum, maxlen, embedding_dim), name='prompt_resh_W')(prompt_drop_x)
    prompt_zcnn = layers.TimeDistributed(layers.Conv1D(
        cnn_filters, cnn_kernel_size, padding='valid'), name='prompt_zcnn')(prompt_resh_W)
    prompt_avg_zcnn = layers.TimeDistributed(
        Attention(), name='prompt_avg_zcnn')(prompt_zcnn)

    prompt_MA_list = MultiHeadAttention(100, num_heads)(prompt_avg_zcnn)
    # prompt_MA_list.shape = (batch_size, seq_len, embedding_dim)
    prompt_MA_lstm_list = layers.LSTM(
        lstm_units, return_sequences=True)(prompt_MA_list)
    prompt_avg_MA_lstm_list = Attention()(prompt_MA_lstm_list)

    query = prompt_avg_MA_lstm_list

    # 여기에서 합쳐진다.
    es_pr_MA_list = [MultiHeadAttention_PE(100, num_heads)(
        pos_avg_MA_lstm_list[i], query) for i in range(output_dim)]
    es_pr_MA_lstm_list = [layers.LSTM(lstm_units, return_sequences=True)(
        pos_hz_MA) for pos_hz_MA in es_pr_MA_list]
    es_pr_avg_lstm_list = [Attention()(pos_hz_lstm)
                           for pos_hz_lstm in es_pr_MA_lstm_list]
    ...
    for index, rep in enumerate(range(output_dim)):
        ...
        final_pred = layers.Dense(
            units=1, activation='sigmoid')(attention_concat)
        final_preds.append(final_pred)

    y = layers.Concatenate()([pred for pred in final_preds])

    model = keras.Model(inputs=[pos_input, prompt_word_input,
                        prompt_pos_input, linguistic_input, readability_input], outputs=y)
    model.summary()
